...
from .fusion import ItemSemanticFusionHead, UserSemanticFusionHead
import numpy as np
...
class LightGCN(nn.Module):
    ...
    def _sparse_mm_amp_safe(self, dense: torch.Tensor) -> torch.Tensor:
        """
        AMP-safe sparse propagation step.

        torch.sparse.mm is not implemented for float16 on some CUDA builds.
        We always execute sparse matmul in float32 with autocast disabled,
        then cast back to the original dtype to keep the rest of the forward
        pass compatible with AMP.
        """
        original_dtype = dense.dtype
        dense_fp32 = dense.float()
        adj_fp32 = self.adj_torch if self.adj_torch.dtype == torch.float32 else self.adj_torch.float()

        if dense.is_cuda:
            with torch.autocast(device_type="cuda", enabled=False):
                out_fp32 = torch.sparse.mm(adj_fp32, dense_fp32)
        else:
            out_fp32 = torch.sparse.mm(adj_fp32, dense_fp32)

        if original_dtype == torch.float32:
            return out_fp32
        return out_fp32.to(original_dtype)
    ...
    def propagate(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for _ in range(self.n_layers):
            g = self._sparse_mm_amp_safe(g)   # [N, d]
            out = out + g 
    ...
    def propagate_with_layers(self) -> list[torch.Tensor]:
        ...
        """
        _, item_layers = self.propagate_with_user_item_layers()
        return item_layers

    def propagate_with_user_item_layers(self) -> tuple[list[torch.Tensor], list[torch.Tensor]]:
        """
        Return user/item embeddings at each GNN layer (including layer 0).

        Returns:
            user_emb_layers: length = n_layers + 1, each [num_users, dim]
            item_emb_layers: length = n_layers + 1, each [num_items, dim]
        """
        g = torch.cat([self.user_embedding.weight, self.item_embedding.weight], dim=0)
        user_layers: list[torch.Tensor] = []
        item_layers: list[torch.Tensor] = []

        user_emb, item_emb = torch.split(g, [self.num_users, self.num_items], dim=0)
        user_layers.append(user_emb)
        item_layers.append(item_emb)

        for _ in range(self.n_layers):
            g = self._sparse_mm_amp_safe(g)
            user_emb, item_emb = torch.split(g, [self.num_users, self.num_items], dim=0)
            user_layers.append(user_emb)
            item_layers.append(item_emb)
...
class LightGCNBudgetedDualSemantic(LightGCNBudgetedSemantic):
    """
    Dual-side budgeted semantic model:
      - item side: selected-node conservative semantic injection
      - user side: cluster-prototype transfer with need-aware gating
    """
    ...
    def __init__(
        ...
        item_semantic_embeddings: dict[int, torch.Tensor] | torch.Tensor,
        selected_item_mask: torch.Tensor,
        item_semantic_dim: int,
        user_semantic_embeddings: dict[int, torch.Tensor] | torch.Tensor,
        selected_user_mask: torch.Tensor,
        user_need_score: torch.Tensor,
        user_semantic_dim: int,
        fusion_hidden_dim: int = 256,
        gate_temperature: float = 1.0,
        max_residual_scale: float = 0.35,
        gate_bias: float = -2.5,
        user_gate_temperature: float | None = None,
        user_max_residual_scale: float | None = None,
        user_gate_bias: float | None = None,
        device: str | torch.device = "cuda",
    ):
        super().__init__(
            num_users=num_users,
            ...
            n_layers=n_layers,
            adj_mat=adj_mat,
            item_semantic_embeddings=item_semantic_embeddings,
        ...
        self.device = torch.device(device)
        self.user_semantic_dim = int(user_semantic_dim)
        self.user_fusion = UserSemanticFusionHead(
            gnn_dim=embedding_dim,
            semantic_dim=self.user_semantic_dim,
            hidden_dim=int(fusion_hidden_dim),
            gate_temperature=float(
                gate_temperature if user_gate_temperature is None else user_gate_temperature
            ),
            max_residual_scale=float(
                max_residual_scale if user_max_residual_scale is None else user_max_residual_scale
            ),
            gate_bias=float(gate_bias if user_gate_bias is None else user_gate_bias),
        )

        user_semantic_matrix = torch.zeros((num_users, self.user_semantic_dim), dtype=torch.float32)
        if isinstance(user_semantic_embeddings, dict):
            iterator = user_semantic_embeddings.items()
        else:
            iterator = enumerate(user_semantic_embeddings)
        for uid, emb in iterator:
            uid = int(uid)
            ...
            if not torch.is_tensor(emb):
                emb = torch.as_tensor(emb)
            emb = emb.view(-1).detach().cpu().float()
            copy_dim = min(self.user_semantic_dim, emb.numel())
            user_semantic_matrix[uid, :copy_dim] = emb[:copy_dim]

        selected_user_mask = torch.as_tensor(selected_user_mask, dtype=torch.float32).view(-1, 1)
        if selected_user_mask.size(0) != num_users:
            raise ValueError(
                f"selected_user_mask length mismatch: {selected_user_mask.size(0)} vs {num_users}"
            )
        user_need_score = torch.as_tensor(user_need_score, dtype=torch.float32).view(-1, 1)
        if user_need_score.size(0) != num_users:
            raise ValueError(
                f"user_need_score length mismatch: {user_need_score.size(0)} vs {num_users}"
            )

        self.register_buffer("user_semantic_emb", user_semantic_matrix)
        self.register_buffer("user_selected_mask", selected_user_mask)
        self.register_buffer("user_need_score", torch.clamp(user_need_score, min=0.0, max=1.0))

    def _fuse_users(
        self,
        user_ids: torch.Tensor,
        user_gnn: torch.Tensor,
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        semantic_emb = self.user_semantic_emb[user_ids].to(user_gnn.device)
        selected_mask = self.user_selected_mask[user_ids].to(user_gnn.device)
        need_score = self.user_need_score[user_ids].to(user_gnn.device)
        fused, sem_proj, alpha, _ = self.user_fusion(
            gnn_emb=user_gnn,
            semantic_emb=semantic_emb,
            selected_mask=selected_mask,
            need_score=need_score,
        )
        return fused, sem_proj, selected_mask, need_score, alpha
    ...
    def forward(
        ...
        user_base = user_g[users]
        pos_base = item_g[pos_items]
        neg_base = item_g[neg_items]

        user_fused, user_sem_proj, user_mask, user_need, user_alpha = self._fuse_users(users, user_base)
        pos_fused, pos_sem_proj, pos_mask, pos_alpha = self._fuse_items(pos_items, pos_base)
        neg_fused, neg_sem_proj, neg_mask, neg_alpha = self._fuse_items(neg_items, neg_base)
    ...
    def predict(self, users: torch.Tensor):
        ...
        all_item_ids = torch.arange(self.num_items, device=item_g.device, dtype=torch.long)
        item_fused, _, _, _ = self._fuse_items(all_item_ids, item_g)
        user_fused, _, _, _, _ = self._fuse_users(users, user_g[users])
